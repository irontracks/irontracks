-- Fix: delete_teacher_cascade referenciava 3 tabelas que NÃO existem mais no schema,
-- então a RPC quebrava em "relation ... does not exist" pra QUALQUER professor —
-- o Postgres resolve a tabela no plano mesmo quando o WHERE não retorna linha.
-- Efeito: a exclusão de professor no painel admin nunca concluía (o client dava
-- "permission denied for table teachers"; a rota daria o erro de relation).
--
-- Removidas (verificado em 2026-07-17 que não existem em public):
--   • assessment_photos  — nenhuma tabela de fotos referencia assessments hoje
--   • invites            — era do Teamwork V2, dropado
--   • messages           — substituída por direct_messages (que a RPC já limpa)
-- Não há dado órfão: as tabelas não existem, então não há o que apagar nelas.
--
-- O resto da função (gate is_admin/service_role, cascata, DELETE FROM teachers,
-- update de profile, audit_events) fica IDÊNTICO ao que estava em produção.
...
BEGIN
  -- Detecta service_role via auth.role() (o GUC request.jwt.claim.role é null no PostgREST atual).
