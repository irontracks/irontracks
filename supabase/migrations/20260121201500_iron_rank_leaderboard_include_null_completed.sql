-- IRON RANK leaderboard: include sets where completed is NULL (legacy rows) as completed.
...
BEGIN
  ...
  legacy_by_workout AS (
    ...
    WHERE NOT EXISTS (SELECT 1 FROM sets_by_workout sbw WHERE sbw.workout_id = w.id)
      ...
      AND COALESCE(public.try_parse_numeric(e.value->>'weight'), 0) > 0
      AND COALESCE(public.try_parse_numeric(e.value->>'reps'), 0) > 0
    GROUP BY w.id, w.user_id
  ),
  lifted AS (
    SELECT
