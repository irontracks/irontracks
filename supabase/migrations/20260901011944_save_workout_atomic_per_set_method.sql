-- `save_workout_atomic` passa a gravar o método por série.
-- Idêntica à versão anterior; muda só o INSERT em public.sets (coluna
-- per_set_method). Sem isso a coluna existiria e nunca seria escrita — o
-- "Salvar no plano" do seletor de método viraria botão mudo.
  ...
  LOOP
    ...
    LOOP
      ...
      INSERT INTO public.sets (
        exercise_id, weight, reps, rpe, set_number, completed, is_warmup, set_type, advanced_config, per_set_method
      ) VALUES (
        v_exercise_id,
        ...
        NULLIF(COALESCE(v_set->>'per_set_method', ''), '')
      );
