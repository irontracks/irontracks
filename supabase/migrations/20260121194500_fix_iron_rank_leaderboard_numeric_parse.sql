-- Fix IRON RANK leaderboard: handle legacy reps/weight like "9-12" (ranges) safely.
...
BEGIN
  ...
  lifted AS (
    SELECT
      wl.user_id,
      SUM(public.try_parse_numeric(e.value->>'weight') * public.try_parse_numeric(e.value->>'reps')) AS total_volume_kg
