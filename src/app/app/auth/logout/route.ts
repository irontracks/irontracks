 ...
 *
 * `/app` (não `/`) — desde a fase 3 da migração raiz↔/comercial (26/09/2026)
 * a raiz nua é a landing comercial, não o app. Redirecionar pra `/` aqui
 * deixaria quem sai da conta olhando pra landing em vez da tela de login.
 */
export async function GET(request: Request) {
  ...
  if (!supabaseUrl || !supabaseAnonKey) {
    return NextResponse.redirect(new URL('/app', safeOrigin))
  }
...
<a href="/app" style="display:block;text-decoration:none;background:#facc15;color:#000;font-weight:900;padding:12px 14px;border-radius:12px">Continuar</a>
</div>
<script>
(function(){
  try {
  ...
  try { window.location.replace('/app') } catch (e) { window.location.href = '/app' }
})();
</script>
</body>
</html>`
