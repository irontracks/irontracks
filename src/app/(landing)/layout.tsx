...
/**
 * Layout da LANDING (raiz do domínio) — hotfix de 26/09/2026.
 *
 * Na fase 3 da migração raiz↔/comercial este CSS e as fontes do Google foram
 * parar no ROOT layout, que envolve TAMBÉM o app (`/app/*`). Resultado medido
 * em produção: toda página do app passou a receber `html, body { height: auto
 * !important; overflow-y: visible !important }` (sobrescrevendo o
 * `height: 100%` do `globals.css` do app) e o `preconnect`/folha do Google
 * Fonts — que o app tinha removido de propósito (viola o `connect-src` do CSP
 * e entrega o IP do usuário ao Google a cada carregamento).
 *
 * Aqui, num grupo de rotas `(landing)` (não muda a URL: `/` continua `/`), o
 * CSS só existe enquanto a landing está montada — o mesmo escopo que ele tinha
 * quando morava em `comercial/layout.tsx`.
 *
 * Guard: src/__tests__/cspPreconnectFontes.test.ts (o layout raiz não pode
 * carregar nada que seja só da landing).
 */
    ...
    <>
      ...
      <style dangerouslySetInnerHTML={{
        __html: `
          ...
          /* Links do menu: escondidos no celular, visíveis a partir de 768px.
             Eram as classes hidden/md:flex do Tailwind — que não carrega na
             landing desde a fase 3 (o Tailwind do projeto vem do CSS do app). */
          .com-nav-links { display: none; align-items: center; }
          @media (min-width: 768px) { .com-nav-links { display: flex; } }
