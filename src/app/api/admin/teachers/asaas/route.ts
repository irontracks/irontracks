...
export async function POST(req: Request) {
  try {
    ...
    if (!teacherRow && email) {
      const { data } = await admin.from('teachers').select('id, email, user_id, asaas_wallet_id').ilike('email', safeEmailLike(email)).maybeSingle()
      teacherRow = data || null
    }
      ...
      if (insertErr || !inserted) {
        ...
        if (!existing) {
          const { data } = await admin
            .from('teachers')
            .select('id, email, user_id, asaas_wallet_id')
            .ilike('email', safeEmailLike(resolvedEmail))
            .maybeSingle()
          existing = data || null
        }
        if (!existing) {
    ...
    return respondInternalError('api:admin:teachers:asaas', e)
  }
}
