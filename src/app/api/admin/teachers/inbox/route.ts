...
const messageTemplate = (kind: string, studentName: string) => {
  ...
  if (kind === 'churn_risk') return `Oi ${name}! Notei que você ficou alguns dias sem treinar. Está tudo certo? Quer que eu ajuste algo no treino pra facilitar sua rotina essa semana?`
  if (kind === 'volume_drop') return `Oi ${name}! Notei uma queda no seu volume de treino nos últimos dias. Como você está se sentindo (sono, energia, dores)? Posso ajustar o plano pra ficar mais sustentável.`
  if (kind === 'load_spike') return `Oi ${name}! Vi um aumento grande de carga/volume recentemente. Só confirmando: está tudo ok (técnica, dores, recuperação)? Se quiser, ajusto pra manter a evolução com segurança.`
  if (kind === 'checkins_alert') return `Oi ${name}! Vi alguns sinais no seu check-in (energia/dor/recuperação). Está tudo certo? Quer que eu ajuste o treino pra ficar mais confortável e sustentável?`
  ...
  try {
    ...
    for (const uid of studentUserIds) {
      ...
      if (daysSince >= cfg.churnDays) {
        ...
        candidates.push({ kind: 'churn_risk', title: 'Risco de churn', reason: lastAt ? `${daysSince} dias sem treinar` : 'Sem treinos registrados', score })
      }
      ...
      if (v.vPrev7 >= cfg.minPrev7Volume && dropRatio <= dropThreshold) {
        ...
        candidates.push({ kind: 'volume_drop', title: 'Queda de volume', reason: `Volume 7d caiu ${pct}% (de ${Math.round(v.vPrev7)} → ${Math.round(v.v7)})`, score: 500 + pct })
      }
      ...
      if (v.vPrev7 >= cfg.minPrev7Volume && incRatio >= spikeThreshold && v.v7 >= cfg.minCurrent7VolumeSpike) {
        ...
        candidates.push({ kind: 'load_spike', title: 'Aumento brusco de carga', reason: `Volume 7d subiu ${pct}% (de ${Math.round(v.vPrev7)} → ${Math.round(v.v7)})`, score: 600 + pct })
      }
      ...
      if (ca) {
        ...
        if (reasons.length) candidates.push({ kind: 'checkins_alert', title: 'Alerta de check-in', reason: reasons.join(' • '), score: 650 + score })
      }
