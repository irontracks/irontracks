...
const PROMPT = [
    ...
    '',
    'O formato da resposta é imposto pelo schema da chamada. Escreva frases curtas e',
    'diretas, dentro dos limites de tamanho de cada campo. Se um campo não se aplicar,',
    'devolva texto vazio em vez de inventar.',
].join('\n')
...
export async function POST(req: Request) {
    try {
        ...
        // Structured output: o schema vai NA CHAMADA. Sem ele o modelo devolvia JSON
        // quebrado/estourando os limites do Zod com frequência (ver aiContract.ts).
        const model = getGeminiModel(
            apiKey,
            env.gemini.modelId,
            bodyPhotoGenerationConfig(LAUDO_RESPONSE_SCHEMA, MAX_OUTPUT_TOKENS),
        )

        let laudo: BodyPhotoLaudo | null = null
        let lastPreview = ''
        for (let attempt = 1; attempt <= MAX_MODEL_ATTEMPTS && !laudo; attempt++) {
            const attemptParts: Part[] = attempt === 1
                ? parts
                : [...parts, { text: '\nATENÇÃO: a resposta anterior veio fora do formato. Responda SOMENTE o JSON do schema, com textos curtos.' }]
            const geminiResult = await safeGemini('body-composition-photo', () => model.generateContent(attemptParts))
            ...
            const normalized = normalizeLaudo(extractJsonFromModelText(rawText))
            const validated = BodyPhotoLaudoSchema.safeParse(normalized)
            if (validated.success) {
                laudo = validated.data
                if (attempt > 1) logWarnRemote('ai:body-composition-photo:retry-ok', 'laudo só passou na 2ª tentativa')
                break
            }
        ...
        if (!laudo) {
            logError('ai:body-composition-photo:invalid', new Error('schema mismatch'), { rawPreview: lastPreview })
            await admin.from('body_photo_assessments').update({ status: 'failed' }).eq('id', assessmentId)
