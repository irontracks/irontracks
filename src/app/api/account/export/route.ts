...
export async function GET() {
  try {
    ...
    const profileRes = await safeSelect('profiles', admin.from('profiles').select('*').eq('id', userId).limit(1))
    ...
    const settingsRes = await safeSelect('user_settings', admin.from('user_settings').select('*').eq('user_id', userId).limit(1))
    if (settingsRes.ok === false) return NextResponse.json({ ok: false, error: getErrorMessage(settingsRes.error) }, { status: 500 })
    outData.userSettings = settingsRes.rows?.[0] || null

    const workoutsRes = await safeSelect('workouts', admin.from('workouts').select('*').eq('user_id', userId).order('created_at', { ascending: false }).limit(500))
    if (workoutsRes.ok === false) return NextResponse.json({ ok: false, error: getErrorMessage(workoutsRes.error) }, { status: 500 })
    ...
    if (exercisesRes.ok === false) return NextResponse.json({ ok: false, error: getErrorMessage(exercisesRes.error) }, { status: 500 })
    ...
    if (setsRes.ok === false) return NextResponse.json({ ok: false, error: getErrorMessage(setsRes.error) }, { status: 500 })
    outData.sets = setsRes.rows || []

    const activeSessionsRes = await safeSelect('active_workout_sessions', admin.from('active_workout_sessions').select('*').eq('user_id', userId).limit(50))
    if (activeSessionsRes.ok === false) return NextResponse.json({ ok: false, error: getErrorMessage(activeSessionsRes.error) }, { status: 500 })
    outData.activeWorkoutSessions = activeSessionsRes.rows || []

    const notificationsRes = await safeSelect('notifications', admin.from('notifications').select('*').eq('user_id', userId).order('created_at', { ascending: false }).limit(500))
    if (notificationsRes.ok === false) return NextResponse.json({ ok: false, error: getErrorMessage(notificationsRes.error) }, { status: 500 })
    outData.notifications = notificationsRes.rows || []

    const messagesRes = await safeSelect('messages', admin.from('messages').select('*').eq('user_id', userId).order('created_at', { ascending: false }).limit(1000))
    if (messagesRes.ok === false) return NextResponse.json({ ok: false, error: getErrorMessage(messagesRes.error) }, { status: 500 })
    ...
    if (directChannelsRes.ok === false) return NextResponse.json({ ok: false, error: getErrorMessage(directChannelsRes.error) }, { status: 500 })
    ...
    if (directMessagesRes.ok === false) return NextResponse.json({ ok: false, error: getErrorMessage(directMessagesRes.error) }, { status: 500 })
    ...
    if (invitesRes.ok === false) return NextResponse.json({ ok: false, error: getErrorMessage(invitesRes.error) }, { status: 500 })
    ...
    if (appointmentsRes.ok === false) return NextResponse.json({ ok: false, error: getErrorMessage(appointmentsRes.error) }, { status: 500 })
    ...
    if (assessmentsRes.ok === false) return NextResponse.json({ ok: false, error: getErrorMessage(assessmentsRes.error) }, { status: 500 })
    ...
    if (photosRes.ok === false) return NextResponse.json({ ok: false, error: getErrorMessage(photosRes.error) }, { status: 500 })
