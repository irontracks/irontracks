...
export class VideoCompositor {
    ...
    private cleanup() {
        ...
        if (this.maxDurationTimer !== null) {
            try { clearTimeout(this.maxDurationTimer); } catch { }
        }
    ...
    public async render({
        ...
        // Safety timeout: force-stop recording at 3× clip duration + 10s to prevent infinite hangs
        const maxDurationMs = Math.max(duration * 3000 + 10000, 15000);
        this.maxDurationTimer = setTimeout(() => {
            logWarn('warn', `VideoCompositor: maxDuration timeout reached (${maxDurationMs}ms). Forcing recorder stop.`);
        ...
        const processFrame = () => {
            if (this.isCancelled) return;
            // Guard: if recorder already stopped (e.g. via maxDuration timeout), bail out
