 ...
 * que cada um já tem. Alimenta o editor.
 *
 * Quando o mesmo nome aparece com horários diferentes em dias diferentes (plano
 * vindo da IA, ou um plano editado antes desta tela existir), vence o PRIMEIRO
 * encontrado: o editor mostra um valor só por nome, e inventar um "vários" aqui
 * só empurraria a ambiguidade para a tela.
 */
export function horariosDoPlano(days: PlanDay[] | null | undefined): Array<{ nome: string; time: string }> {
  const out: Array<{ nome: string; time: string }> = []
  ...
  for (const dia of Array.isArray(days) ? days : []) {
    for (const meal of Array.isArray(dia?.meals) ? dia.meals : []) {
      ...
      const atual = out[idx]
      if (atual && !atual.time && time) atual.time = time
    }
  }
  return out
}
