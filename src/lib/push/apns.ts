...
export async function sendPushToUsers(
    ...
    try {
        ...
        // ── Android (FCM) — fire-and-forget in parallel ──────────────────────
        let fcmPromise: Promise<unknown> = Promise.resolve()
        try {
            // Dynamic import to avoid breaking if fcm.ts has issues
            const { sendFcmToUsers } = await import('./fcm')
            fcmPromise = sendFcmToUsers(ids, title, body, extra)
                .then((fcmResults) => {
                    fcmResults.forEach((r) => results.push(r))
                })
                .catch(() => { /* FCM is best-effort */ })
        } catch { /* FCM module not available — skip */ }
