 ...
 * @module useWhatsNew
 *
 * Detects unreleased "What's New" entries since the user's last viewed
 * version and shows a notification badge + modal. Persists the last-seen
 * version in user settings so the banner only appears once per release.
 *
 * @returns `{ pending, dismiss, hasUpdate }`
 */
'use client'
