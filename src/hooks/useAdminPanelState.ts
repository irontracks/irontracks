/**
 * @module useAdminPanelState
 *
 * Controls open/close state and active tab of the admin panel.
 * Persists selection to localStorage so the panel remembers its last
 * state across page navigations. Only activates for users with admin role.
 *
 * @returns `{ showAdminPanel, togglePanel, activeTab, setActiveTab }`
 */
'use client'
