            ...
            if (onSave) {
                ...
                const exercisesPayload = (workout.exercises || []).map((ex, idx) => {
                    ...
                    for (let i = 0; i < numSets; i += 1) {
                        ...
                        sets.push({ weight: s?.weight ?? null, reps: s?.reps ?? ex?.reps ?? null, rpe: s?.rpe ?? ex?.rpe ?? null, set_number: s?.set_number ?? (i + 1), completed: false, is_warmup: !!(s?.is_warmup ?? s?.isWarmup), advanced_config: s?.advanced_config ?? s?.advancedConfig ?? null, ...perSetMethodField(s), ...duracaoDaSerieField(s) })
                    }
