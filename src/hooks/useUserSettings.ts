 ...
 *
 * Reads and writes the per-user settings object from the `profiles` table.
 * Provides a typed `settings` state with defaults, a `patchSettings` method
 * for partial updates, and auto-sync to the database on change.
 * Validates against `UserSettingsSchema` to prevent malformed data.
 *
 * @returns `{ settings, patchSettings, loading, error }`
 */
'use client'
    ...
    try {
      if (typeof window !== 'undefined') {
        ...
        if (cached && typeof cached === 'object') {
          // Note: the earlier force-reset of restTimerAutoStart=false has been
          // removed. Auto-advance is back (opt-in via Settings) and wired to
          // `autoStartEnabled` in RestTimerOverlay — the old race condition
          // ("START disparou sozinho") is gated by a per-rest fired-ref so
          // toggling AUTO after isFinished can't retro-fire the current rest.
