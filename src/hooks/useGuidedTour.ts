/**
 * @module useGuidedTour
 *
 * Manages the multi-step onboarding tour overlay. Tracks the current step,
 * stores completion state in user settings, and provides `next`, `skip`,
 * and `dismiss` controls. Automatically advances through tour steps and
 * persists progress so the tour doesn't restart on page reload.
 *
 * @param userId - Current user ID
 * @returns `{ tourState, currentStep, next, skip, dismiss }`
 */
'use client'
