...
export const TeamWorkoutProvider = ({ children, user, settings, onStartSession }: TeamWorkoutProviderProps) => {
    ...
    /**
     * A feature volta LIGADA para todos (restauração de 17/08/2026, pedido do
     * dono). Antes ela vivia atrás de `featureTeamworkV2`, uma flag com default
     * FALSE — restaurar com ela seria devolver a feature invisível, que é o
     * oposto do que foi pedido. O sistema de flags, aliás, já não existe: foi
     * removido no #436, depois da aposentadoria daqui.
     *
     * O que continua respeitando a vontade do usuário é `allowTeamInvites`
     * (acima): quem desliga não RECEBE convite — mas segue podendo convidar.
     */
    const teamworkV2Enabled = true;
