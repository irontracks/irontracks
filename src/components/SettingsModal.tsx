      ...
      <div className="w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl" style={{ background: 'rgba(12,12,12,0.99)', border: '1px solid rgba(234,179,8,0.22)', boxShadow: '0 0 40px rgba(234,179,8,0.10), 0 30px 80px rgba(0,0,0,0.65)' }}>
        ...
        <div className="p-4 space-y-3 max-h-[75vh] overflow-y-auto custom-scrollbar">
          ...
          <div className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            <div className="space-y-2">
              <button type="button" onClick={() => { try { const s = encodeURIComponent('Suporte IronTracks'); const b = encodeURIComponent('Olá equipe IronTracks,\n\nPreciso de ajuda com:\n\n'); window.open(`mailto:irontrackscompany@gmail.com?subject=${s}&body=${b}`, '_blank') } catch { } }} className="w-full flex items-center gap-3 p-3 rounded-xl bg-neutral-900 border border-neutral-700 hover:bg-neutral-800 hover:border-neutral-600 transition-colors group">
              ...
              <button type="button" onClick={() => { try { window.open('https://tawk.to/chat/irontracks', '_blank') } catch { } }} className="w-full flex items-center gap-3 p-3 rounded-xl bg-neutral-900 border border-neutral-700 hover:bg-neutral-800 hover:border-neutral-600 transition-colors group">
          ...
          <div className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              <button type="button" onClick={async () => { try { const payload = JSON.stringify(draft && typeof draft === 'object' ? draft : {}, null, 2); const blob = new Blob([payload], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `irontracks-settings-${new Date().toISOString()}.json`; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url) } catch (e: unknown) { await alert('Falha ao exportar: ' + (getErrorMessage(e) ?? String(e))) } }} className="min-h-[44px] px-4 py-3 rounded-xl bg-neutral-900 border border-neutral-700 text-neutral-200 font-black hover:bg-neutral-800 inline-flex items-center justify-center gap-2">
                <Download size={16} className="text-yellow-500" /> Exportar
              </button>
              <button type="button" onClick={async () => { try { setDraft(DEFAULT_SETTINGS); await alert('Preferências resetadas. Clique em Salvar para aplicar.') } catch (e: unknown) { await alert('Falha ao resetar: ' + (getErrorMessage(e) ?? String(e))) } }} className="min-h-[44px] px-4 py-3 rounded-xl bg-neutral-900 border border-neutral-700 text-neutral-200 font-black hover:bg-neutral-800 inline-flex items-center justify-center gap-2">
                <RotateCcw size={16} className="text-yellow-500" /> Resetar
              </button>
              <button type="button" onClick={async () => { try { if (typeof window === 'undefined') return; const keys: string[] = []; for (let i = 0; i < window.localStorage.length; i += 1) { const k = window.localStorage.key(i); if (k) keys.push(k) } keys.forEach((k) => { if (k.startsWith('irontracks.')) { try { window.localStorage.removeItem(k) } catch { } } }); try { window.location.reload() } catch { } } catch (e: unknown) { await alert('Falha ao limpar cache: ' + (getErrorMessage(e) ?? String(e))) } }} className="min-h-[44px] px-4 py-3 rounded-xl bg-neutral-900 border border-neutral-700 text-neutral-200 font-black hover:bg-neutral-800 inline-flex items-center justify-center gap-2">
                <Trash2 size={16} className="text-yellow-500" /> Limpar cache
              </button>
            </div>
            <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
              <button type="button" onClick={async () => { try { const res = await fetch('/api/account/export', { method: 'GET' }); const data = await res.json().catch((): null => null); if (!data || !data.ok) { await alert('Falha ao exportar dados: ' + (data?.error || '')); return } const payload = JSON.stringify(data, null, 2); const blob = new Blob([payload], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `irontracks-account-export-${new Date().toISOString()}.json`; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url) } catch (e: unknown) { await alert('Falha ao exportar dados: ' + (getErrorMessage(e) ?? String(e))) } }} className="min-h-[44px] px-4 py-3 rounded-xl bg-neutral-900 border border-neutral-700 text-neutral-200 font-black hover:bg-neutral-800 inline-flex items-center justify-center gap-2">
            ...
            <div className="mt-3">
              <button type="button" onClick={async () => { try { const typed = typeof window !== 'undefined' ? window.prompt('Digite EXCLUIR para confirmar a exclusão da conta:') : null; if (String(typed || '').trim().toUpperCase() !== 'EXCLUIR') return; const res = await fetch('/api/account/delete', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ confirm: 'EXCLUIR' }) }); const data = await res.json().catch((): null => null); if (!data || !data.ok) { await alert('Falha ao excluir conta: ' + (data?.error || '')); return } try { let supa; try { supa = createClient() } catch { supa = null } if (supa) await supa.auth.signOut({ scope: 'global' }) } catch { } try { window.location.href = '/auth/signin' } catch { } } catch (e: unknown) { await alert('Falha ao excluir conta: ' + (getErrorMessage(e) ?? String(e))) } }} className="w-full min-h-[44px] px-4 py-3 rounded-xl bg-red-600/15 border border-red-500/40 text-red-200 font-black hover:bg-red-600/25 inline-flex items-center justify-center gap-2">
        ...
        <div className="p-4 border-t border-neutral-800 bg-neutral-900/50 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            <button type="button" onClick={() => props?.onClose?.()} className="px-4 py-3 rounded-xl bg-neutral-800 border border-neutral-700 text-neutral-200 font-bold hover:bg-neutral-700 transition-colors">Fechar</button>
            <button type="button" disabled={!canSave} onClick={async () => { try { const ok = await props?.onSave?.(draft); if (ok === false) return; props?.onClose?.() } catch (e: unknown) { await alert('Falha ao salvar: ' + (getErrorMessage(e) ?? String(e))) } }} className={canSave ? 'px-4 py-3 rounded-xl bg-yellow-500 text-black font-black hover:bg-yellow-400 inline-flex items-center gap-2 transition-colors' : 'px-4 py-3 rounded-xl bg-yellow-500/70 text-black font-black cursor-wait inline-flex items-center gap-2'}>
              <Save size={16} />
