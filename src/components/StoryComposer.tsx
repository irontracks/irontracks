...
export default function StoryComposer({ open, session, onClose }: StoryComposerProps) {
  ...
  const renderVideoFrameAsJpeg = (): Promise<{ blob: Blob; filename: string; mime: string }> => {
    ...
    return new Promise<{ blob: Blob; filename: string; mime: string }>((resolve, reject) =>
      canvas.toBlob(
        (b) => b
          ? resolve({ blob: b, filename: `irontracks-story-${Date.now()}.jpg`, mime: 'image/jpeg' })
    ...
    try {
      ...
      const result = await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error('render_timeout_60s')), RENDER_TIMEOUT_MS)
        )
      ])

      // Validate the result blob is not empty
      if (!result.blob || result.blob.size === 0) {
        logWarn('StoryComposer', 'Compositor returned empty blob, falling back to JPEG')
        throw new Error('empty_blob')
      }

      return result
    } catch (e) {
      // Cancel compositor if still running
      if (compositorRef.current) {
        try { compositorRef.current.cancel() } catch { }
      }

      logWarn('StoryComposer', 'Video render failed, falling back to JPEG frame', e)

      // Fallback: render a static JPEG of the current frame with overlay
      try {
        const fallback = await renderVideoFrameAsJpeg()
        setInfo('Exportação de vídeo não disponível. Imagem salva com o relatório.')
        return fallback
      } catch (fallbackErr) {
        logError('error', 'JPEG fallback also failed', fallbackErr)
