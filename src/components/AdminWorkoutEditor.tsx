...
const AdminWorkoutEditor = ({ initialData, onSave, onCancel }) => {
    const [workout, setWorkout] = useState(() => {
        ...
        const exercises = Array.isArray(source.exercises)
            ? source.exercises.filter((ex) => ex && typeof ex === 'object')
            : [];
        return {
            ...source,
            id: source.id ?? null,
    ...
    const addExercise = () => {
        setWorkout((prev) => {
            ...
            return { ...current, exercises: [...currentExercises, { 
            name: '', 
            sets: 4, 
            reps: '10', 
            rpe: '8', // Default RPE
    ...
    return (
        <div className="bg-neutral-900 min-h-full">
            ...
            <input value={workout?.title || ''} onChange={e => setWorkout((prev) => ({ ...(prev || {}), title: e.target.value }))} className="w-full bg-neutral-800 text-xl font-bold p-4 rounded-xl mb-4 border border-neutral-700 outline-none" placeholder="Nome do Treino"/>
            ...
            <div className="space-y-4">
                {safeExercises.map((ex, idx) => (
                    <div key={idx} className="bg-neutral-800 p-4 rounded-xl border border-neutral-700 relative group">
                        <button onClick={() => { const n = [...safeExercises]; n.splice(idx, 1); setWorkout((prev) => ({ ...(prev || {}), exercises: n })); }} className="absolute top-2 right-2 text-red-500 p-2"><Trash2 size={16}/></button>
