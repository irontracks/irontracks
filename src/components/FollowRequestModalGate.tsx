...
export default function FollowRequestModalGate(props) {
  ...
  const setAppNotification = typeof props?.setNotification === 'function' ? props.setNotification : null
  const supabase = useMemo(() => createClient(), [])
  const [open, setOpen] = useState(false)
  const [notification, setNotification] = useState<any>(null)
  const [followerProfile, setFollowerProfile] = useState<any>(null)
  ...
  const openWithNotification = useCallback(async (n) => {
    ...
    const displayName = String(prof?.display_name || '').trim() || 'Alguém'
    const title = String(safe?.title || '').trim() || 'Solicitação para seguir'
    const message = String(safe?.message || '').trim() || `${displayName} quer te seguir.`
    const photoURL = String(prof?.photo_url || '').trim() || null

    if (view === 'active' && setAppNotification) {
      const key = `${String(safe?.id || '')}:${fid}`
      if (key && lastToastKeyRef.current === key) return
    ...
    setOpen(true)
  }, [loadFollowerProfile, setAppNotification, view])
