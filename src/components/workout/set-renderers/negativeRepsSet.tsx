...
const NegativeRepsSetInner = ({ ex, exIdx, setIdx }: { ex: WorkoutExercise; exIdx: number; setIdx: number }) => {
  const { getLog, updateLog, setNegativeRepsModal, openNotesKeys, toggleNotes, startTimer, reportHistory , settings} = useWorkoutContext();
  ...
  return (
    <div key={key} className="space-y-1">
      ...
      >
        {done ? (
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] uppercase tracking-widest font-black text-emerald-400 shrink-0">Negativas</span>
      ...
      {!done && !canDone && <div className="pl-12 text-[11px] text-neutral-400 font-semibold">Preencha peso, reps e tempo excêntrico no modal.</div>}
