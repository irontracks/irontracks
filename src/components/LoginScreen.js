...
const LoginScreen = () => {
    ...
    const getLoginHref = (provider = 'google') => {
        try {
            ...
            return `/auth/login?provider=${encodeURIComponent(provider)}&next=${encodeURIComponent(nextPath)}`;
        } catch {
            return `/auth/login?provider=${encodeURIComponent(provider)}&next=${encodeURIComponent('/dashboard')}`;
        }
    };
    ...
    const handleLogin = async (provider = 'google') => {
        ...
        try {
            const href = getLoginHref(provider);
        ...
        <div className="relative flex flex-col items-center justify-center min-h-[100dvh] overflow-hidden bg-neutral-950 text-white p-6">
            ...
            <div className="relative z-10 w-full max-w-sm p-8 rounded-[2rem] border border-white/5 bg-neutral-900/40 backdrop-blur-xl shadow-2xl shadow-black/50 flex flex-col items-center">
                ...
                <button
                    ...
                    disabled={!!loadingProvider}
                    className="mt-3 group w-full flex items-center justify-center gap-3 bg-neutral-900 border border-neutral-700 text-white px-6 py-4 rounded-xl font-black text-lg hover:bg-neutral-800 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    {loadingProvider === 'apple' ? (
                        ...
                        <>
                            ...
                            <span className="tracking-tight">Entrar com Apple</span>
                        </>
                    )}
                </button>
