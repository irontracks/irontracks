...
import {
    ...
    CheckCircle2, CalendarDays, Phone, ShieldCheck, Trophy, Crown, Trash2
} from 'lucide-react';
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="flex flex-wrap items-center gap-2">
                                    <button
                                        ...
                                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-yellow-500/10 hover:bg-yellow-500/20 border border-yellow-500/25 text-yellow-400 text-[11px] font-black uppercase tracking-wider transition-all active:scale-95"
                                    >
                                        <Edit3 size={11} />
                                        Editar
                                    </button>
                                    {deleteConfirmId === t.id ? (
                                        <button
                                            ...
                                            onClick={async () => {
                                                ...
                                                try {
                                                    await handleDeleteTeacher(t.id, () => setSelectedTeacher(null));
                                                } finally {
                                                    setDeletingTeacher(false);
                                                    setDeleteConfirmId(null);
                                                }
                                            }}
                                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-red-600 hover:bg-red-500 border border-red-500 text-white text-[11px] font-black uppercase tracking-wider transition-all active:scale-95 disabled:opacity-60"
                                        >
                                            {deletingTeacher ? <Loader2 size={11} className="animate-spin" /> : <Trash2 size={11} />}
                                            Confirmar?
                                        </button>
                                    ) : (
                                        <button
                                            onClick={() => setDeleteConfirmId(t.id)}
                                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-red-500/10 hover:bg-red-500/20 border border-red-500/25 text-red-400 text-[11px] font-black uppercase tracking-wider transition-all active:scale-95"
                                        >
                                            <Trash2 size={11} />
                                            Excluir
                                        </button>
