...
export function SettingsNotificationsSection({ draft, setValue, iosNotifStatus, iosNotifBusy, iosTimeSensitiveStatus, onRequestIosNotifPermission, onOpenAppSettings, isIosNative }: SettingsNotificationsSectionProps) {
    ...
    const notifyWorkoutUpdated = Boolean(draft?.notifyWorkoutUpdated ?? true)
    const notifyDietPlan = Boolean(draft?.notifyDietPlan ?? true)
        ...
        <SectionCard>
            ...
            <div className="space-y-3">
                ...
                <div className="pt-3 border-t border-neutral-700/60 space-y-3">
                    ...
                    />
                    <NotifRow
                        title="Treino ajustado pelo professor"
                        description="Avisa você quando seu professor mexe num treino que você já tem."
                        checked={notifyWorkoutUpdated}
                        disabled={!pushNotificationsEnabled}
                        onChange={() => setValue('notifyWorkoutUpdated', !notifyWorkoutUpdated)}
                    />
                    <NotifRow
                        title="Dieta do professor"
                        description="Avisa você quando seu professor prescreve ou ajusta seu plano alimentar."
                        checked={notifyDietPlan}
                        disabled={!pushNotificationsEnabled}
                        onChange={() => setValue('notifyDietPlan', !notifyDietPlan)}
                    />
                    <NotifRow
        ...
        <div className="fixed inset-0 z-[1400] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 pt-safe" role="dialog" aria-modal="true" aria-label="Personalizar módulos" onClick={onClose} onKeyDown={(e) => e.key === 'Escape' && onClose?.()}>
            ...
            <div className="w-full max-w-md bg-neutral-900 border border-neutral-800 rounded-2xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
                <div className="p-4 border-b border-neutral-800 flex items-center justify-between">
                    <div><div className="t-meta-inherit text-xs text-yellow-500">Módulos</div><div className="text-white font-black text-lg truncate">Personalizar</div></div>
