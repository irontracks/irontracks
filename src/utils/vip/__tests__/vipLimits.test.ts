...
describe('FREE_LIMITS e UNLIMITED_LIMITS (constantes públicas)', () => {
  ...
  it('FREE_LIMITS: sem analytics, offline, lab_exams, workout_photo_import', () => {
    expect(FREE_LIMITS.analytics).toBe(false)
    expect(FREE_LIMITS.offline).toBe(false)
    expect(FREE_LIMITS.lab_exams).toBe(false)
    expect(FREE_LIMITS.workout_photo_import).toBe(false)
  })

  it('UNLIMITED_LIMITS: chat_daily=9999 e todos os booleans (incl. lab_exams, workout_photo_import) true', () => {
    ...
    expect(UNLIMITED_LIMITS.workout_photo_import).toBe(true)
  })

  it('ambas as constantes têm a chave lab_exams e workout_photo_import (guard anti-drift do shape)', () => {
    expect(FREE_LIMITS).toHaveProperty('lab_exams')
    expect(UNLIMITED_LIMITS).toHaveProperty('lab_exams')
    expect(FREE_LIMITS).toHaveProperty('workout_photo_import')
    expect(UNLIMITED_LIMITS).toHaveProperty('workout_photo_import')
  })
})
...
describe('applyTierDefaults', () => {
  it('vip_elite: ativa nutrition_macros, analytics, offline, lab_exams e workout_photo_import', () => {
    ...
    expect(limits.workout_photo_import).toBe(true)
  })

  it('vip_pro: ativa offline, lab_exams e workout_photo_import, mas não analytics', () => {
  ...
  it('vip_start: ativa lab_exams e workout_photo_import (features básicas de todo VIP)', () => {
  ...
  it('qualquer plano vip_* desconhecido ainda recebe lab_exams e workout_photo_import (não fica sem por omissão)', () => {
...
describe('applyTierCaps', () => {
  describe('vip_start', () => {
    it('caps: chat 10, wizard 1, insights 3, history 60; sem macros/analytics; lab_exams e workout_photo_import true', () => {
      ...
      expect(l.workout_photo_import).toBe(true)
    })
  })

  describe('vip_pro', () => {
    it('caps: chat 40, history ilimitado, macros/offline/lab_exams/workout_photo_import on, analytics off', () => {
 ...
 * `analytics` (só true no Elite) em vez de `workout_photo_import` — VIP
 * Start/Pro pagante via o import de ficha por foto/PDF bloqueado do mesmo
 * jeito que o free. Trava a CLASSE: toda feature "básica de todo VIP" (como
 * lab_exams já era) precisa estar true nos três tiers pagos.
 */
describe('workout_photo_import é feature básica de TODO VIP (não só Elite)', () => {
  it.each(['vip_start', 'vip_pro', 'vip_elite'])('%s: workout_photo_import=true', (tier) => {
    const l = applyTierCaps(tier, applyTierDefaults(tier, { ...FREE_LIMITS }))
    expect(l.workout_photo_import).toBe(true)
  })
})
