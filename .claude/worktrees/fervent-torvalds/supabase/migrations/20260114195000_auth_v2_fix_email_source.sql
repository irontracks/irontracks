...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.students s WHERE lower(s.email) = v_email)
     AND NOT EXISTS (SELECT 1 FROM public.teachers t WHERE lower(t.email) = v_email) THEN
