...
- **⚠️ A chave Gemini do `.env.local` é a MESMA de produção e está em FREE TIER: 20 requisições por DIA por modelo** (`GenerateRequestsPerDayPerProjectPerModel-FreeTier`). Rodar ~20 chamadas de teste local **derruba TODAS as features de IA do app pro dia inteiro** — aconteceu em 31/07/2026 e a Avaliação por Foto parou pros usuários. Antes de fazer medição empírica contra a API, combine com o dono ou use chave separada. Diagnóstico de IA em produção: runtime logs da Vercel (MCP `get_runtime_logs`) — **o Sentry NÃO recebe erros de rota server** hoje.
