...
**Treino em dupla (TeamworkV2) — REMOVIDO, não é bug.** A feature foi **aposentada de propósito em 15/07/2026**, código e banco, com confirmação do dono. Investigado e fechado em 02/08/2026 — a nota anterior aqui dizia "as tabelas sumiram, suspeita de perda na reescrita de histórico" e isso estava **errado**. Cronologia real: PR #428 (15/07 00:45) deletou 22 arquivos / ~4,6k linhas do client (`contexts/team/*`, `TeamWorkoutContext`, componentes, rotas `api/team/*`) deixando o banco intacto de propósito; PR #430 varreu o resíduo; PR #437 (15/07 15:47) aplicou a migration `20260715153440_drop_teamwork_v2_and_feature_flags.sql`, que dropou `invites`, `team_sessions`, `team_session_presence`, `team_chat_messages`, as RPCs `accept_team_invite`/`leave_team_session`/`can_view_team_session` (CASCADE levou junto as policies de `realtime.messages` do broadcast) e a tabela `feature_flags` (o sistema de flags saiu no PR #436 — `utils/featureFlags.ts` não existe mais). **Não há nada quebrado em produção: sem client chamando, não há "relation does not exist".** Nada disso deve ser recriado sem uma decisão nova do dono.

Resíduos conhecidos e inofensivos, se um dia incomodarem: os tipos gerados em `src/types/supabase.ts` ainda declaram as 4 tabelas + `can_view_team_session` (basta regerar), e 28 linhas de `user_settings.preferences` ainda guardam a chave `featureTeamworkV2` (1 com `true`) — sem leitor no código. **PR #506** ("canal `team_logs` privado") é um PR **zumbi**: trabalho de 11/07, recriado pela reescrita de histórico, que reintroduz `contexts/team/useTeamBroadcast.ts` e a migration `20260711125043_team_logs_broadcast_realtime_authz.sql` de uma feature que já não existe — deve ser **fechado**, não mergeado.
